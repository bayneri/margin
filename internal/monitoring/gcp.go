...
func buildDashboard(req ApplyDashboardRequest) *dashboardpb.Dashboard {
	...
	tiles = append(tiles, tile(0, y, columns, 2, dashboardIntro(req.Dashboard.Service, req.Dashboard.Runbook)))
	y += 2
...
func dashboardIntro(service, runbook string) *dashboardpb.Widget {
	...
	if strings.TrimSpace(runbook) != "" {
		content = fmt.Sprintf("%s\n\nRunbook: [%s](%s)", content, runbook, runbook)
	}
